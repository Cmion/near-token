...
const MAX_MINTABLE_TOKEN: u128 = 1000_000_000_000_000_000_000_000_000;
#[near_bindgen]
...
impl Contract {
    ...
    pub fn new(
        ...
        // Registers the contract account with the token.
        contract.token.internal_register_account(owner_id.as_ref());
        // Deposits the total supply to the owner's account.
        contract
            .token
    ...
    pub fn get_account(&self, account_id: AccountId) -> u128 {
        assert_self();
        self.token
            .accounts
            .get(&account_id)
            .unwrap_or_else(|| env::panic(b"Account not found"))
    }

    #[payable]
    pub fn ft_mint(&mut self, receiver_id: AccountId, amount: U128) {
        assert!(
            amount.0 <= MAX_MINTABLE_TOKEN,
